...
public class transform35Test {
    ...
    @Test
    public void process4test35habr() throws IOException, TransformationException, NoSuchAlgorithmException, ParserConfigurationException, SAXException, CanonicalizationException, InvalidCanonicalizerException {
        ...
        transxml.xmldsig("xml4test/input.xml", "xml4test/input.xml2");
        InputStream in = new FileInputStream("xml4test/input.xml2");
        OutputStream out = new FileOutputStream("xml4test/out.xml");
        test.process(in, out);
        File f = new File("xml4test/out.xml");
        BufferedReader b = new BufferedReader(new FileReader(f));
